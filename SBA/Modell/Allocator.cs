...
public class Allocator
{
    ...
    private Quay quay;
    private int[] s; // Zeitpunkt, an dem Schiff i am Quai anlegt
    private int[] b; // Liegeposition von Schiff i
    ...
    public Allocator(List<Ship> ships, Quay quay)
    {
        this.ships = ships;
        this.quay = quay;
        s = new int[ships.Count];
        b = new int[ships.Count];
    ...
    {
        if (sp1 == sp2 || (sp2 < sp1 && sp1 < ep2) || (ep2 > ep1 && ep1 > sp2))
        {
            return true;
        }
        return false;
    ...
    {
        int furthestPositionInLine = quay.Capacity - v.Size;
        int newPosition = quay.Capacity - v.Size;
        List<int> positionsInLine = new List<int>();

        if (b[v.ID] + v.Size == quay.Capacity)
            return b[v.ID];
        foreach (Ship ship in A)
        {
        ...
        foreach (var intervals in forbiddenTimes)
        {
            foreach (var interval in intervals.Value)
            {
                ...
                {
                    if (PosOverlap(intervals.Key, quay.Capacity, newPosition, newPosition + v.Size))
                    {
